...
import { GameState, EducationalFact, MuseumSection, AccessibilitySettings } from '../types';
import { EDUCATIONAL_FACTS, MUSEUM_SECTIONS, JOURNAL_DATA, UI_TRANSLATIONS, CREDITS_DATA } from '../constants';

const UI: React.FC = () => {
  const { 
      state, score, distance, collectibles, lives, highScore, countdown, 
      narrative, nearMiss, startGame, resetGame, setGameState, activeBlessing, 
  ...
  // Helper for UI Text - SAFE version
  const t = (key: string) => {
              ...
              <div className="relative w-full max-w-4xl aspect-[4/3] bg-[#fdfbf7] rounded-lg shadow-2xl overflow-hidden flex flex-col border-8 border-[#8B4513]">
                  ...
                  <div className="bg-[#8B4513] p-4 flex justify-between items-center text-[#E6C288] border-b border-[#5e3b25] z-20">
                      ...
                      <div className="flex gap-4">
                          <span className="font-serif italic text-sm">{unlockedJournalPages.length} / {JOURNAL_DATA.length} {t('COMPLETED')}</span>
                          <button onClick={() => setGameState(GameState.PAUSED)} className="hover:text-white font-bold">✕</button>
                      </div>
                  </div>
                  ...
                  <div className="flex-1 flex relative">
                      ...
                      <div className="flex-1 p-8 flex flex-col justify-center">
                          ...
                          {/* Navigation Controls (Bottom Right/Left depending on RTL) */}
      ...
      <div className={`absolute top-0 left-0 w-full h-full pointer-events-none p-4 flex flex-col justify-between z-10 ${textScale}`}>
        ...
        <div className="flex justify-between items-start w-full">
            <div className="flex flex-col gap-2 pointer-events-auto">
                ...
                {activeBlessing && (
                    <div className="bg-yellow-500/20 backdrop-blur-sm p-3 rounded-lg border border-yellow-400 text-yellow-100 shadow-[0_0_15px_rgba(255,215,0,0.3)] animate-pulse">
                        <p className={`font-title tracking-wider text-yellow-300 ${largeTextClass}`}>{t('BLESSING_ACTIVE')}</p>
                        <p className="text-sm font-bold">{activeBlessing.name}</p>
                    </div>
                )}
            </div>
      ...
      return (
          <div className="absolute inset-0 z-50 flex">
              <div className="w-1/3 h-full bg-[#1a1410] border-r border-[#8B4513] p-6 flex flex-col overflow-y-auto">
                  <h2 className="text-3xl text-[#FFD700] font-title mb-6 border-b border-[#8B4513] pb-4">{t('MUSEUM')}</h2>
                  ...
                  <button onClick={() => setGameState(GameState.MENU)} className="mt-auto py-3 border border-[#8B4513] text-[#D4A574] hover:bg-[#2F1B10] transition-colors">
                      {t('EXIT_MUSEUM')}
                  </button>
              </div>
  ...
  if (state === GameState.MENU) {
    return (
      <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black/60 z-20 backdrop-blur-sm">
        <div className="bg-[#1a1410] border-2 border-[#E6C288] p-8 max-w-md w-full text-center relative overflow-hidden shadow-2xl rounded-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setGameState(GameState.EDUCATION)} className="w-full py-3 bg-transparent hover:bg-[#E6C288]/10 text-[#E6C288] border border-[#E6C288]/50 transition-all font-arabic text-sm">
                    {t('HISTORY')}
                </button>
                <button onClick={toggleMuseum} className="w-full py-3 bg-[#2F1B10] hover:bg-[#4a2c1a] text-[#FFD700] border border-[#FFD700]/50 transition-all font-title text-sm flex items-center justify-center gap-1">
